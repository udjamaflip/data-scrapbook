...
- **Commit private infrastructure details** — URLs, hostnames, IP addresses, internal service names, or credentials of any kind belong in `.env` / local config only, never in committed files
- **Assume code files are clean** — always scan files for PII, credentials, internal URLs, and UGC before staging; this includes scripts, notebooks, config files, and inline comments
- **Commit PII or user-generated content** — names, emails, phone numbers, addresses, account IDs, or any data that could identify a real person must never be committed, even as examples or test fixtures
